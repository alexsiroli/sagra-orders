...
const mockUser = {
  uid: 'user1',
  email: 'cassa@test.com',
  displayName: 'Cassa Test',
};
  ...
  return render(
    React.createElement(BrowserRouter, null,
      React.createElement(AuthProvider, null,
        React.createElement(Cassa, null)
      )
    )
  );
};
...
describe('Cassa Integration Tests', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  test('should handle mock user data structure', () => {
    expect(mockUser.uid).toBe('user1');
    expect(mockUser.email).toBe('cassa@test.com');
    expect(mockUser.displayName).toBe('Cassa Test');
  });
});
